...
class SubjectBulkUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        created = []
        existed = set(name.lower() for name in Subject.objects.filter(school=request.user.school).values_list('name', flat=True))

        for row in sheet.iter_rows(min_row=2, values_only=True):
            name = row[0]
            if name and name.lower() not in existed:
                created.append(Subject(name=name.strip(), school=request.user.school))
                existed.add(name.lower())

        if created:
            Subject.objects.bulk_create(created)
            return Response({"message": f"{len(created)} subjects uploaded successfully."}, status=200)
        else:
            return Response({"message": "No new subjects to upload. All are duplicates."}, status=200)
