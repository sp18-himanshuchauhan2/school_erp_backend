...
class ClassroomSubjectAdmin(admin.ModelAdmin):
    ...
    search_fields = ['subject_name',
                     'classroom__class_name', 'teacher__user__first_name']
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if not request.user.is_superuser:
            try:
                ...
                form.base_fields['classroom'].queryset = Classroom.objects.filter(
                    school=school)
                form.base_fields['subject'].queryset = Subject.objects.filter(
                    school=school)
                form.base_fields['teacher'].queryset = Teacher.objects.filter(
                    user__school=school)
